...
- Open VSCode and then use `File > Open Folder` to open the project folder e.g. `hello-world` folder on your computer (@VSCodePythonDocs, @MicrosoftPythonVSCode2023).
...
- Now, you are ready to explore the coding fun in VSCode. 
...
There are several formats to add to add documentation to your program, some most popular ones are (@AutodocstringExtension):

- google
- sphinx
- pep257

We will use a custom.template to  add documentation to your program. Download the [custom template](custom.mustache) (right-click and save) and follow the steps to add it to VSCode. 

1. After downloading the custom.mustache file, open the `Settings editor`, use the following VSCode menu command:
    - On Windows/Linux - File > Preferences > Settings
    - On macOS - Code > Preferences > Settings
2. When you open the Settings editor, you can search and discover the settings to look for `AutoDocstring`.

:::{.callout-warning}
If you cannot find the AutoDocstring in the settings, you missed installing the AutoDocstring extension, so go back to [previous page](python_extensions.html) for instructions.
:::
